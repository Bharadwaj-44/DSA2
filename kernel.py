...
IPYKERNEL = os.environ.get('IPYKERNEL', 'dsa')
...
class CodeKernel(object):
    def __init__(self,
        ...
        # Set environment variable for custom plt.show() function
        os.environ['DSA_SESSION_CACHE_PATH'] = session_cache_path
        ...
        self.idle_timeout_seconds = 30 * 60  # 30 minutes
    ...
    def execute_code_(self, code):
        # Ensure kernel is alive (restart if idle-shutdown)
        self.ensure_alive()
        # Update activity timestamp
        self.update_activity()
        
        # Inject custom plt.show function if matplotlib is being used
        if 'matplotlib' in code or 'plt.' in code or 'seaborn' in code or 'sns.' in code:
            custom_show_code = """
import os
import time
import hashlib
...
"""
    ...
    def check_idle_timeout(self):
        """Check if kernel has been idle too long and should shutdown"""
        if self.is_shutdown:
            return True
        
        idle_time = time.time() - self.last_activity_time
        if idle_time > self.idle_timeout_seconds:
            print(f" [KERNEL] Idle for {idle_time/60:.1f} minutes, shutting down...")
            self.shutdown()
    ...
    def ensure_alive(self):
        """Restart kernel if it was shutdown due to idle timeout"""
        if self.is_shutdown:
            print(" [KERNEL] Restarting kernel after idle shutdown...")
            self.start()
            self.is_shutdown = False
            self.update_activity()
